...
async fn main(){
    ...
    let df = CsvReadOptions::default().with_has_header(true)
        ...
        .expect("Failed to read csv file").finish()
        .expect("Failed to read csv file");
    println!("{:?}", df.shape());
    println!("{:?}", df.head(Some(5)));
    println!("{:?}", df.schema());
    let selection = df
        .select_series(["title", "score", "num_comments"]).expect("Failed to select columns");
    let df = DataFrame::new(selection).expect("Failed to create DataFrame")
        .drop_nulls::<String>(None).expect("Failed to drop null values")
        .sort(["score"], SortMultipleOptions::default().with_order_reversed()).expect("Failed to sort DataFrame");
    println!("{:?}", df.head(Some(5)));
